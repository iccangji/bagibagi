...
function itemCount()
{
    if (auth()->check()) {
        ...
        $count = Cart::whereNotNull('session_id')->where('session_id', session('cartSessionId'))->sum('quantity');
    }

    return $count;
}

function checkPickedLottery($lottery, $ticket)
{
    $pickedTickets = $lottery->pickedTickets?->map(fn($item) => $item['choosen_tickets'])->flatten()->toArray();
    $isAvailable   = empty(array_intersect($ticket, $pickedTickets));
    return $isAvailable;
}

function checkCartLottery($cartTickets, $ticket)
{
    $isSelected = !empty(array_intersect($ticket, $cartTickets));
    return $isSelected;
}

function pickedTicket($lottery, $ticket)
{
    $pickedTickets = $lottery->pickedTickets?->map(fn($item) => $item['choosen_tickets'])->flatten()->toArray();
    $tickets       = array_values(array_intersect($ticket, $pickedTickets));
    return $tickets;
}
